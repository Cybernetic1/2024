...
As Curry-Howard suggest that propositions are some kind of ``spaces'', can they have topological structure?  The genius Voevodsky, who recently died young, proposed that there is a homotopy structure in proposition space, which is the basic idea of HoTT (homotopy type theory).
}
\begin{equation}
...
According to my superficial understanding, a proposition either has a proof or does not have a proof. If there is a proof, there is no difference between proof A and proof B. But HoTT proposes that they can differ. In the internal space of a proposition, two path-connected points (proofs) are regarded as identical, but there can be spatial structures that are not path-connected in this space, so the two proofs can be regarded as different.
}
...
An example is: The ancient Westerners regarded Venus as the morning star and evening star, without knowing that they were actually the same star. This is the difference between extension and extension (the meaning is different from the actual extension), which involves the issue of intensional logic. Its semantics can be processed by modal logic and Montague semantics. For details, please refer to this article : https://plato.stanford.edu/entries/logic-intensional/
}
...
For another example, the same group can have different group presentations, and it seems that it can also be handled by HoTT.
}

\cc{这些 不是 path-connected 的空间 具有 groupoid 结构，可以产生 1-groupoid，2-groupoid 等不同的层级，直至 ∞-groupoid. 这方面我暂时不太理解。
}{
These spaces that are not path-connected have a groupoid structure, which can produce different levels such as 1-groupoid, 2-groupoid, etc., until ∞-groupoid. I don't understand this aspect at the moment.
}

\cc{大家可以看到，HoTT 涉及的是「真」的 内部（即黄色方格），但 AGI 主要涉及的是 逻辑的应用，是在 命题空间的 外部（即整个黄色香蕉那空间、其子集空间
}{
As you can see, HoTT involves the interior of "truth" (i.e., the yellow square), but AGI mainly involves the application of logic, which is outside the proposition space (i.e., the entire yellow banana space and its subset space)
}

\cc{之间的映射）。 这并不是说 HoTT 对 AGI 没用，但它的影响是比较 微妙 (subtle) 的，我暂时不能判断。
}{
mapping between). This is not to say that HoTT is useless for AGI, but its impact is subtle and I cannot judge it yet.
}
...
In fact, Transformer has the ability to learn very complex syntactic manipulations, that is to say, it can implicitly learn the derivation methods of various logics we study, such as modal logic. If so, it seems that it can be "bypassed" without requiring us to directly implement those troublesome logical forms. But it is also possible that we artificially attach certain logical structure constraints that can accelerate deep learning. These need to be confirmed experimentally and are very important directions at present.
}
