...
We want to compare the \textbf{hypothesis spaces} $A$ and $B$:  $A$ = unconstrained, fully-connected neural network with $L$ layers and $N$ total weights.  The parameter space $\Theta_A = \mathbb{R}^N$.  $B$ = symmetric neural network with a special structure but its parameter space is also $\Theta_B = \mathbb{R}^M$.

\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.7]{no-free-lunch.png}}}
\end{equation}

It's easier to consider $A$ and $B$ as both embedded in an ambient function space $\mathcal{H}$ that is ``fine'' enough to contain all points of $A$ and $B$ and to measure the ``roughness'' of their landscapes:
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.7]{no-free-lunch-2.png}}}
\end{equation}
